...
class MessageHandler {
  ...
  async handleIncomingMessage(message, senderInfo) {
    if (message?.type === 'text') {
      ...
      if(this.isGreeting(incomingMessage)){
        console.log("lo que es incomingMessage: ", incomingMessage );
        console.log("lo que se envia a sendWelcomeMessage", message.from, message.id, senderInfo );
  ...
  async sendWelcomeMessage(to, messageId, senderInfo) {
    console.log("Que es to", to )
    const name = this.getSenderName(senderInfo);
    const welcomeMessage = `Hola ${name}, Bienvenido a tu centro XXX de confianza, Tu lugar favorito. ¿En qué puedo ayudarte hoy?`;
  ...
  async handleAppointmentFlow(to, message) {
    ...
    console.log("message en appointment flow: ", message);
    console.log("state en appointment flow: ", state);
    console.log("to en appointment flow: ", to);  
  ...
  async handleAssistandFlow(to, message) {
    ...
    if (state.step === 'question') {
      //response = await openAiService(message); //toca pagar 
